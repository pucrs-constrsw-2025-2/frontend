...
export async function getLessons(params?: { page?: number; limit?: number; code?: string; name?: string; }): Promise<LessonListResponse> {
  const qs = new URLSearchParams();
  if (params?.page) qs.append('page', String(params.page));
  if (params?.limit) qs.append('limit', String(params.limit));
  if (params?.code) qs.append('code', params.code);
  if (params?.name) qs.append('name', params.name);
  return apiRequest<LessonListResponse>(`/lessons${qs.toString() ? `?${qs.toString()}` : ''}`);
}
